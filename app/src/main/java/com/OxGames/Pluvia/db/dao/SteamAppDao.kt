...
interface SteamAppDao {
    ...
    @Query("UPDATE steam_app SET drm_check_timestamp = :timestamp WHERE id = :appId")
    suspend fun updateDrmTimestamp(appId: Int, timestamp: Long)

    @Query("SELECT * FROM steam_app WHERE id = :appId LIMIT 1")
